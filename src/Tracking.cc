...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    // // Yolo
    // cv::Mat InputImage;
    // InputImage = imRGB.clone();// 克隆RGB图像
    // mpDetector->GetImage(InputImage);// 从图像中获取检测区域
    // mpDetector->Detect();// 进行目标检测
    // mpORBextractorLeft->mvDynamicArea = mpDetector->mvDynamicArea;// 将动态区域信息传递给ORB特征提取器
    // {
    //     std::unique_lock<std::mutex> lock(mpViewer->mMutexPAFinsh);// 使用互斥锁保护关键帧显示
    //     mpViewer->mmDetectMap = mpDetector->mmDetectMap;// 将检测结果传递给关键帧显示
    // }
    // mpDetector->mvDynamicArea.clear();// 清空动态区域信息

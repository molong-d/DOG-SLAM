     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    //*******
    ExtractORB_depth(0,imGray,0,0,imDepth);//加深度
...
void Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1)
{
    ...
    if(flag==0)
        ...
        monoLeft = (*mpORBextractorLeft)(im,cv::Mat(),mvKeys,mDescriptors,vLapping); //原
        // monoLeft = (*mpORBextractorLeft)(im,im,mvKeys,mDescriptors,vLapping);
...
//**************
void Frame::ExtractORB_depth(int flag, const cv::Mat &im, const int x0, const int x1,const cv::Mat &dm)
{
    vector<int> vLapping = {x0,x1};
    // 判断是左图还是右图
    if(flag==0)
        ...
        // 左图的话就套使用左图指定的特征点提取器，并将提取结果保存到对应的变量中 
        // monoLeft = (*mpORBextractorLeft)(im,cv::Mat(),mvKeys,mDescriptors,vLapping); //原
        monoLeft = (*mpORBextractorLeft)(im,dm,mvKeys,mDescriptors,vLapping);
    else
        // 右图的话就需要使用右图指定的特征点提取器，并将提取结果保存到对应的变量中 
        monoRight = (*mpORBextractorRight)(im,cv::Mat(),mvKeysRight,mDescriptorsRight,vLapping);
}
